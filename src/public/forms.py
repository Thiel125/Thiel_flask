...
class secti(Form):
    vyrobek = TextField("vloz jmeno vyrobku", validators=[InputRequired(message="vyzadovano")])
    ks = IntegerField("Vloz pocet kusu", validators=[InputRequired(message="vyzadovano")])
    cena = IntegerField("Vloz cenu", validators=[InputRequired(message="vyzadovano")])
    DPH = IntegerField("Zadej hodnotu DPH", validators=[InputRequired(message="vyzadovano")])
...
class RychlostForm(Form):

    SPZ = TextField('SPZ', validators=[
        Predicate(safe_characters, message="Please use only letters (a-z) and numbers"),
        Length(min=7, max=7, message="Please use between 6 and 30 characters"),
        InputRequired(message="You can't leave this empty")
    ])
    rychlost = FloatField('Rychlost')